...
| R56 Desktop builds on macOS, Linux, and Windows, with macOS lifecycle E2E and unsigned packaging | local macOS arm64 production build, lifecycle E2E, Fuse inspection, and unsigned ZIP/DMG make passed; `.github/workflows/ci.yml` adds cross-platform Desktop verification; `.github/workflows/desktop-release.yml` publishes native macOS arm64/x64 DMG/ZIP and Windows Setup.exe assets with checksums for `desktop-v*` tags | Partial |
