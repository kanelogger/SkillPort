...
| R56 Desktop builds on macOS, Linux, and Windows, with macOS lifecycle E2E and unsigned packaging | local macOS arm64 production build, lifecycle E2E, Fuse inspection, and unsigned ZIP make passed; `.github/workflows/ci.yml` adds cross-platform Desktop verification; `.github/workflows/desktop-release.yml` builds native macOS arm64/x64 DMG and ZIP, Windows Setup.exe, and Linux DEB/RPM assets, then publishes checksums and installers for `desktop-v*` tags | Partial |
