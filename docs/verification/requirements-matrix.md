...
| R52 `sklp uninstall` confirms with exact `y`, removes Hub-recorded Agent entries, Hub state and npm-global CLI, and preserves Hub-external linked sources | `tests/uninstall.test.js` covers cancellation, missing Hub, copied and linked Skill cleanup, project/global entries, locator cleanup, unreadable state, a replaced user directory, Hub-lock failure with continued npm removal, npm success/failure, and Chinese output. `scripts/smoke/npm-install.mjs` verifies a packed installation removes its Hub and isolated global executable through the npm CLI fallback. [CI run 29388487227](https://github.com/kanelogger/SkillPort/actions/runs/29388487227) passed this smoke on macOS, Linux, and Windows. | Passed |
| R53 Desktop reuses the current Hub without changing CLI contracts or SQLite schema | `src/application/desktop-skill-port.ts`; 4 Desktop facade tests; 105-test CLI regression suite; packed CLI lifecycle smoke | Passed |
...
| R58 Desktop exposes manual Git update check, preview, and confirmed single/batch update without broadening RPC path authority | Desktop facade update lifecycle test; RPC validation/allowlist tests; Electron E2E verifies Git revision/content update and retained enablement; CLI Git/batch update regression suite covers pins, skips, recovery, and partial failures | Passed |
