...
6. Use **Sync sources** to preview and reconcile Skills added to or removed from registered Git source collections.
7. Use **Export catalog** to choose an HTML output through the system save dialog and create an offline-searchable catalog.
...
**Sync sources** reconciles every Git collection already registered in the shared Hub. It discovers newly added Skills, refreshes existing Skills, and reports upstream-missing Skills. Missing local copies are retained by default. Enable pruning to remove only safe missing copies; enabled missing Skills stay protected until the separate force option is selected. Changing either option invalidates the previous preview, so the Desktop requires another preview before synchronization. Git Skills installed before source collections were introduced still require one explicit `sklp sync <repo> --path <path>` adoption through the CLI.
...
The Desktop does not include background update or source sync checks, repair, self-uninstallation, Developer ID signing, notarization, or automatic application updates.
